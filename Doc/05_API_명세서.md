...
| 엔드포인트 | 문제 | 문서 |
|---|---|---|
| `POST /api/trading/orders` | 클라이언트 가격 신뢰 · 음수 수량 · 잠금 없음 | `11` C-1,C-2,C-4 |
| `GET /api/ranking` | `balance` 계산 오류로 순위가 거꾸로 | `11` C-3 |
| `GET /api/trading/portfolio` | `total_value`가 평가금액이 아닌 매입금액 | `11` M-8 |
| `GET /api/ai/propensity` | 결과를 생성하는 주체가 없어 항상 null | `08` §4-3 |
| `GET /api/ai/recommend` | 종목 추천 — 성격 변경 또는 삭제 권고 | `08` §2 |
| `GET /api/stocks` | 로그인 필수 → 비로그인 둘러보기 불가 | `03` §6 |
| `GET /api/news/*` | 크롤링 직접 호출 → DB 조회로 전환 | `08` §5 |
