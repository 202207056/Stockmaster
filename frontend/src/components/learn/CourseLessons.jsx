...
function LessonPlayer({ lesson, scope }) {
  ...
  return <LearningCard title={`${lesson.id} · ${lesson.title}`} aside={<Link to="/learn?tab=courses" className="text-sm text-gray-500 underline">투자 기초 과정</Link>}>
    ...
    <ol className="mb-5 flex flex-wrap gap-2 text-xs">{['개념 먼저 읽기', '사례로 개념 확인', '짧게 확인', '다음에 적용'].map((label, i) => <li key={label} aria-current={step === i ? 'step' : undefined} className={`rounded-full px-3 py-1.5 ${step === i ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-500'}`}>{i + 1}. {label}</li>)}</ol>
    {step === 0 && <article className="space-y-5 text-sm leading-7 text-gray-700"><div className="rounded-xl bg-brand-50 p-5"><p className="text-base font-bold leading-relaxed text-gray-900">{explanation.answer}</p></div><h3 className="font-bold text-gray-900">왜 그런가요?</h3>{explanation.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}<div className="border-l-4 border-brand-500 pl-4"><h3 className="font-bold">기억할 한 가지</h3><p>{explanation.takeaway}</p></div><section className="rounded-xl bg-gray-50 p-4"><h3 className="font-bold">다음 활동에서 확인할 것</h3><p>{PRACTICE_GUIDES[lesson.id].goal}</p><p className="mt-2 text-xs text-gray-500">계산 예시는 직접 구성한 교육용 가상 상황이며, 별도 표시가 없으면 비용을 제외합니다.</p></section><footer className="text-xs leading-relaxed text-gray-500"><p className="mb-2 font-bold">참고 자료 · 2026.09.16 확인</p><ul className="space-y-2">{explanation.sources.map((id) => <li key={id}><a href={LEARNING_SOURCES[id].url} target="_blank" rel="noopener noreferrer" className="underline">{LEARNING_SOURCES[id].title} ↗</a></li>)}</ul><p className="mt-2">일반 투자 원리를 한국어로 정리했습니다. 해외 시장의 세금·주문 제도를 국내에 그대로 적용하는 설명은 아닙니다.</p></footer></article>}
    {step === 1 && <GuidedPractice key={lesson.id} lesson={lesson} onEvidence={saveEvidence} />}
    ...
    <div className="mt-6 flex flex-wrap gap-2"><LearningButton secondary disabled={step === 0} onClick={() => update({ step: step - 1 })}>이전</LearningButton>{step < 3 && <LearningButton onClick={() => update({ step: step + 1 })}>{step === 0 ? `${PRACTICE_GUIDES[lesson.id].title} →` : step === 1 ? '확인 문제로 이해 점검하기 →' : '다음'}</LearningButton>}{step === 3 && progress.completed && nextLesson && <Link to={`/learn?tab=courses&lesson=${nextLesson.id}`} className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-bold text-brand-700">다음 수업: {nextLesson.title} →</Link>}</div><StorageNotice state={state} />
  </LearningCard>;
}
